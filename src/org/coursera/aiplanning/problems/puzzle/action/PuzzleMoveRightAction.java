 ...
 *
 */
public class PuzzleMoveRightAction extends PuzzleAction 
{
	/**
	 * 
	 */
	public PuzzleMoveRightAction() {
		super(PuzzleActionType.MOVE_RIGHT);
	}
	...
	public boolean isApplicable(PuzzleSearchNode node) 
	{
		...
		if (node.getGeneratorAction() != null && node.getGeneratorAction().equals(PuzzleActionType.MOVE_LEFT)) {
			applicable = false;
		}
		else {
			// get state
			...
			if (column >= (state.getMatrixSize() - 1)) {
				applicable = false;
			}
		}
		return applicable;
